...
class MediaPipePoseAnalyzer:
    ...
    def _aggregate_results(
        ...
        if keep_per_frame:
            aggregated["per_frame"] = frame_metrics

        return aggregated

    def get_feature_dict(self, video_path: str, keep_per_frame: bool = False) -> Dict[str, Any]:
        try:
            results = self.analyze_video(video_path, keep_per_frame=keep_per_frame)
            return {
...
def extract_mediapipe_pose_features(video_path: str, device: str = "cpu", keep_per_frame: bool = False) -> Dict[str, Any]:
    analyzer = MediaPipePoseAnalyzer(device=device)
    return analyzer.get_feature_dict(video_path, keep_per_frame=keep_per_frame)